...
from PyQt5.QtGui import QImage, QPixmap, QPainter, QLinearGradient, QColor, QPen, QFont, QFontDatabase
    ...
    def initUI(self):
        ...
        if font_id == -1:
            ...
            if not font_families:
                print("Font loaded, but no font families found.")
                return
            font_family = font_families[0]
            print(f"Font loaded with ID: {font_id}. Font Family: {font_family}")

        # Set the stylesheet with the loaded font
        self.setStyleSheet(f"""
            QWidget {{ background-color: white; }} 
            QLabel {{ font-size: 72pt; font-family: {font_family}; color: black; }}
        """)
